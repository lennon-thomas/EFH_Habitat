...
all_guam_catch<-merge(guam_reef@data,guam,by.x="NewZONE",by.y="Area_FK", all = TRUE)

guam_catch_map<-tm_shape(all_guam_catch)+
...
guam_substrate_map<-
  ...
  tm_raster(showNA=FALSE,title="Substrate Type",palette="-YlOrRd",legend.show = TRUE) 
...
penc_guam_depth[penc_guam_depth < 0 ]<-0
penc_guam_depth[penc_guam_depth > 5 ]<-0
...
writeRaster(penc_guam_s_cells,filename = paste0(boxdir,"final_results/Guam_pencillatus_EFH.tif"),overwrite = TRUE)

penc_guam_s_cells_area<-area(penc_guam_s_cells,na.rm = TRUE)

penc_guam_zonal<-as.data.frame(penc_guam_s_cells_area)

write.csv(penc_guam_zonal,paste0(boxdir,"final_results/Guam_pencillatus_EFG_df.csv"))
